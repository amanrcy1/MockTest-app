...
export const DashboardSkeleton = () => (
  <div className="space-y-5 animate-pulse">
      ...
      {[1, 2, 3].map((i) => (
        <div key={i} className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700">
          <div className="flex flex-col items-center">
            <div className={`w-10 h-10 bg-gray-200 dark:bg-gray-700 rounded-xl mb-2 ${shimmerClass}`} />
    ...
    <div className="space-y-3">
      ...
      {[1, 2, 3].map((i) => (
        <div key={i} className="bg-white dark:bg-gray-800 rounded-xl p-4 flex items-center gap-4 border border-gray-100 dark:border-gray-700">
    ...
    <div className="animate-pulse">
      ...
      <div className="bg-gray-50 dark:bg-gray-900 p-4 flex gap-4 border-b border-gray-200 dark:border-gray-700">
        {Array.from({ length: cols }).map((_, i) => (
          <div key={i} className={`h-4 bg-gray-200 dark:bg-gray-700 rounded flex-1 ${shimmerClass}`} />
        ))}
      </div>
      
      {/* Rows */}
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div key={rowIndex} className="p-4 border-b border-gray-100 dark:border-gray-700 last:border-0 flex gap-4">
          {Array.from({ length: cols }).map((_, colIndex) => (
            <div key={colIndex} className={`h-4 bg-gray-200 dark:bg-gray-700 rounded flex-1 ${shimmerClass}`} />
          ))}
        </div>
      ))}
    </div>
...
export const ListSkeleton = ({ items = 5 }) => (
  <div className="space-y-3">
    {Array.from({ length: items }).map((_, i) => (
      <div key={i} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 animate-pulse border border-gray-100 dark:border-gray-700">
        <div className="flex items-center gap-4">
          <div className={`h-12 w-12 bg-gray-200 dark:bg-gray-700 rounded-full flex-shrink-0 ${shimmerClass}`} />
          <div className="flex-1 space-y-2">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[1, 2, 3].map((j) => (
            <div key={j} className="text-center">
              <div className={`h-6 w-12 mx-auto bg-gray-200 dark:bg-gray-700 rounded mb-1 ${shimmerClass}`} />
              <div className={`h-3 w-16 mx-auto bg-gray-200 dark:bg-gray-700 rounded ${shimmerClass}`} />
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="grid grid-cols-3 gap-4">
      {[1, 2, 3].map((i) => (
        <div key={i} className="bg-white dark:bg-gray-800 rounded-xl p-4 text-center border border-gray-100 dark:border-gray-700">
          <div className={`h-8 w-16 mx-auto bg-gray-200 dark:bg-gray-700 rounded mb-2 ${shimmerClass}`} />
          <div className={`h-3 w-12 mx-auto bg-gray-200 dark:bg-gray-700 rounded ${shimmerClass}`} />
        </div>
      ))}
    </div>
