...
const getConfidenceInfo = (meta, boundary) => {
  if (boundary?.violation)
    return { label: 'Low', tone: 'text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20' };
  if (meta?.verified)
    return {
      label: 'High',
  ...
  return {
    label: 'Medium',
    tone: 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20',
  };
};
        ...
        {editing ? (
          ...
          <>
            <motion.div
              ...
              style={{ transformStyle: 'preserve-3d' }}
              onClick={() => {
                if (window.innerWidth < 768 && !loading) setShowMenu((p) => !p);
              }}
              onContextMenu={(e) => {
            ...
            {!loading && (
              <div className="hidden md:flex items-center gap-1 px-1 opacity-0 group-hover/user:opacity-100 transition-opacity">
                {msg.timestamp && (
                  <span className="text-[10px] text-gray-400 dark:text-gray-600 select-none mr-1">
            ...
            <AnimatePresence>
              {showMenu && (
                ...
                >
                  <button
                    onClick={handleCopy}
                    className="flex items-center gap-2.5 w-full px-3.5 py-2.5 text-sm text-gray-700 dark:text-gray-200 active:bg-gray-100 dark:active:bg-gray-700 transition-colors leading-none"
                  >
                    {copied ? (
                      <svg
                  ...
                  <button
                    ...
                    className="flex items-center gap-2.5 w-full px-3.5 py-2.5 text-sm text-gray-700 dark:text-gray-200 active:bg-gray-100 dark:active:bg-gray-700 transition-colors border-t border-gray-100 dark:border-gray-700"
                  >
                    <svg
                      className="w-4 h-4 text-gray-400 flex-shrink-0"
                      fill="none"
            ...
            {msg.timestamp && (
              <span className="md:hidden text-[10px] text-gray-400 dark:text-gray-600 px-1 select-none">
                {formatTime(msg.timestamp)}
              </span>
            )}
          </>
    ...
    >
      ...
      <div className="flex flex-col gap-0.5 max-w-[80%]">
        ...
        >
          {speaking && highlightedParts ? (
            <p className="whitespace-pre-wrap leading-relaxed break-words text-sm">
              ...
              <span className="bg-violet-200 dark:bg-violet-700 text-violet-900 dark:text-white rounded px-0.5">
                {highlightedParts.word}
              </span>
              <span>{highlightedParts.after}</span>
            </p>
          ) : (
            <Suspense
              fallback={<p className="whitespace-pre-wrap leading-relaxed">{msg.content}</p>}
            >
        ...
        <div className="flex flex-wrap items-center gap-1 px-1 mt-0.5 min-h-[24px]">
          {msg.timestamp && (
            <span className="text-[10px] text-gray-400 dark:text-gray-600 select-none leading-none">
              {formatTime(msg.timestamp)}
            </span>
          )}
          {msg.elapsed && !speaking && (
            <span className="text-[10px] text-gray-400 dark:text-gray-500 select-none leading-none">
              · {msg.elapsed}s
            </span>
          )}
          {speaking && (
            <span className="text-[10px] text-violet-500 dark:text-violet-400 select-none font-medium leading-none">
              {Math.floor(speakElapsed / 60)}:{String(speakElapsed % 60).padStart(2, '0')}
            </span>
          )}
          {!speaking && (
            <>
              <span
                className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${getConfidenceInfo(msg.meta, msg.boundary).tone}`}
              >
              ...
              <div className="flex items-center gap-1">
                {getSourceChips(msg.meta, msg.context).map((chip) => (
                  <span
                    key={`${msg.timestamp}-${chip}`}
                    className="text-[10px] px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-300"
                  >
                    {chip}
                  </span>
                ))}
...
const AiChatWidget = memo(({ context = {} }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (cached) {
        ...
        if (uid === currentUser.uid && Date.now() - ts < STATS_CACHE_TTL) {
          setUserStats(data);
          return;
        }
      }
      ...
      try {
        ...
        try {
          sessionStorage.setItem(
            STATS_CACHE_KEY,
            JSON.stringify({ data: statsData, ts: Date.now(), uid: currentUser.uid })
          );
        } catch {
          /* quota */
        }
  ...
  const enrichedContext = useMemo(() => {
    const ctx = { ...context };
    if (userDetails) {
      ctx.userName = userDetails.name || null;
      ctx.examType = userDetails.targetExam || context.examType || null;
    }
    if (userStats?.attempted > 0) {
      ...
      if (userStats.strongSubjects?.length)
        parts.push(`Strong in: ${userStats.strongSubjects.join(', ')}`);
  ...
  const stopListening = useCallback(() => {
    ...
    if (sttSilenceRef.current) {
      clearTimeout(sttSilenceRef.current);
      sttSilenceRef.current = null;
    }
  ...
  const handleSend = useCallback(async () => {
    ...
    setMessages((prev) => [
      ...
      { role: 'user', content: sanitized, timestamp: Date.now() },
    ]);
    setInput('');
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev.slice(-(MAX_MESSAGES - 1)),
        {
          ...
          timestamp: Date.now(),
          elapsed,
          boundary: aiResult.boundary || null,
          meta: aiResult.meta || null,
          context: { examType: enrichedContext.examType || null },
        },
      ]);
    ...
    calc: (
      ...
      >
        <path
          ...
          strokeLinejoin="round"
          strokeWidth={2}
          d="M4 7v10c0 2 1 3 3 3h10c2 0 3-1 3-3V7c0-2-1-3-3-3H7C5 4 4 5 4 7zm4 0h8M8 11h2m4 0h2M8 15h2m4 0h2"
        />
      </svg>
    ),
  };
        ...
        {isOpen && (
          ...
          >
            ...
            >
              ...
              {/* Left: avatar + title */}
              <div
                className={`relative flex items-center transition-all duration-300 ease-out ${keyboardVisible ? 'gap-2 py-1.5 pt-[max(0.375rem,env(safe-area-inset-top))]' : 'gap-3 py-3 pt-[calc(0.75rem+env(safe-area-inset-top,0px))]'}`}
              ...
              <div
                className={`relative flex items-center gap-0.5 transition-all duration-300 ${keyboardVisible ? 'py-1.5' : 'py-3'}`}
                style={{ transform: 'translateZ(10px)' }}
              >
                {messages.length > 0 && (
                  <motion.button
                  ...
                  >
                    <svg
                      className={`transition-all duration-300 ${keyboardVisible ? 'w-3.5 h-3.5' : 'w-4 h-4'}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                >
                  <svg
                    className={`transition-all duration-300 ${keyboardVisible ? 'w-4 h-4' : 'w-5 h-5'}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
            ...
            >
              ...
              {messages.length === 0 && !keyboardVisible && (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <svg className="w-5 h-5 inline -mt-0.5" viewBox="0 0 24 24" fill="none">
                        <path
                          d="M12 2l2.09 6.26L20 6l-3.26 4.91L23 12l-6.26 1.09L20 18l-5.91-2.26L12 22l-2.09-6.26L4 18l3.26-4.91L1 12l6.26-1.09L4 6l5.91 2.26L12 2z"
                          fill="url(#sparkGrad)"
                        />
                        <defs>
                          <linearGradient
                  ...
                  {/* 3D Suggestion chips */}
                  <div
                    className="mt-4 w-full grid grid-cols-2 gap-2 px-1"
                    style={{ perspective: '600px' }}
                  >
                    {suggestions.map((s, i) => (
                      <motion.button
                        ...
                        style={{ transformStyle: 'preserve-3d' }}
                      >
                        <span className="flex-shrink-0" style={{ transform: 'translateZ(6px)' }}>
                          {s.icon}
                        </span>
                        <span className="leading-tight" style={{ transform: 'translateZ(4px)' }}>
                          {s.text}
                        </span>
                      </motion.button>
                    ))}
              ...
              <AnimatePresence>
                {!keyboardVisible && (
                  ...
                  >
                    ...
                    <span className="text-[10px] text-gray-400 dark:text-gray-600 select-none">
                      AI Â· May not always be accurate
                    </span>
                  </motion.div>
                )}
              </AnimatePresence>
