...
const AdminUsers = () => {
  ...
  const handleConfirmToggle = async () => {
    ...
    try {
      ...
      logAdminAction({
        ...
        action: action === 'promote' ? 'promoteUser' : 'demoteUser',
        targetId: user.id,
      });
      setUsers((prev) =>
        prev.map((item) => (item.id === user.id ? { ...item, isAdmin: newAdminStatus } : item))
      );
  ...
  const handleDeleteUser = async (user) => {
    try {
      ...
      const collections = ['tests', 'bookmarks', 'errorReports'];
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            ...
            {filteredUsers.map((user) => (
              <div key={user.id} className="border-t dark:border-gray-700">
                {/* Desktop Row */}
                <div className="hidden md:grid grid-cols-4 gap-4 px-6 py-3 text-sm text-gray-700 dark:text-gray-300 items-center">
                  ...
                  {isSuperAdmin ? (
                    <button
                      ...
                      disabled={
                        updatingId === user.id || user.id === currentUser?.uid || user.isSuperAdmin
                      }
                  ...
                  {isSuperAdmin && (
                    <button
                      ...
                      disabled={
                        deletingId === user.id || user.id === currentUser?.uid || user.isSuperAdmin
                      }
                      className={`px-3 py-1 rounded text-xs font-semibold ${
