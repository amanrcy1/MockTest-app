...
const AdminBookmarks = () => {
  ...
  const handleMarkReviewed = async (bookmarkId) => {
    try {
      ...
      setBookmarks((prev) =>
        prev.map((item) => (item.id === bookmarkId ? { ...item, reviewed: true } : item))
      );
      logAdminAction({
        adminId: userDetails?.userId,
        action: 'reviewBookmark',
        targetId: bookmarkId,
      });
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-950">
      <nav className="bg-white dark:bg-gray-900 shadow-md">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-blue-600">Bookmarks Review</h1>
            ...
            {filteredBookmarks.map((item) => (
              <div key={item.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div>
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {item.examType || 'Exam'} - {item.question?.subject || 'Subject'} -{' '}
                  ...
                  {Array.isArray(item.adminNoteHistory) && item.adminNoteHistory.length > 0 && (
