...
const STATUS_CONFIG = {
  completed: { label: "Completed", color: "text-green-600 dark:text-green-400", bg: "bg-green-50 dark:bg-green-900/20", border: "border-green-200 dark:border-green-800", icon: "M5 13l4 4L19 7" },
  available: { label: "Ready", color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-50 dark:bg-blue-900/20", border: "border-blue-200 dark:border-blue-800", icon: "M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" },
  "on-break": { label: "On Break", color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-50 dark:bg-amber-900/20", border: "border-amber-200 dark:border-amber-800", icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" },
  locked: { label: "Locked", color: "text-gray-400 dark:text-gray-500", bg: "bg-gray-50 dark:bg-gray-800", border: "border-gray-200 dark:border-gray-700", icon: "M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" },
};
...
const PaperSelection = () => {
  ...
  const saveSessionData = useCallback((completed, scores, timestamps, skipped) => {
  ...
  useEffect(() => {
    ...
    const { sectionId, score, correct, incorrect, skipped, accuracy } = location.state.completedPaper;
    ...
    const newScores = { ...existing.scores, [sectionId]: { score, correct, incorrect, skipped, accuracy } };
  ...
  const handleStartPaper = (section, index) => {
    ...
    if (status === "locked") {
      toast.error(`Not enough questions for ${section.name}. Need ${section.totalQuestions}, have ${questionCounts[section.id] || 0}.`);
      return;
    }
    if (status === "completed") {
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm">
              ...
              <p className="text-blue-700 dark:text-blue-300 mt-1">
                ...
                Each paper has its own timer. A 1-hour break is given between papers, but you can skip it and start early.
              </p>
            </div>
          </div>
        </motion.div>
        ...
        {completedCount > 0 && (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Session Progress</h3>
            ...
            return (
              ...
              >
                <div
                  ...
                  onClick={() => { if (status === "available") handleStartPaper(section, index); }}
                  onKeyDown={(e) => { if ((e.key === "Enter" || e.key === " ") && status === "available") { e.preventDefault(); handleStartPaper(section, index); } }}
                ...
                >
                  <div className="flex items-start gap-4">
                    {/* Icon */}
                    <div className={`p-3 rounded-xl ${status === "available" ? "bg-blue-100 dark:bg-blue-800 text-blue-600 dark:text-blue-300" : status === "completed" ? "bg-green-100 dark:bg-green-800 text-green-600 dark:text-green-300" : "bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500"}`}>
                      {icon}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className={`font-bold text-lg ${status === "locked" ? "text-gray-400 dark:text-gray-500" : "text-gray-900 dark:text-white"}`}>
                      ...
                      {status === "completed" && score && (
                        <div className="mt-3 flex items-center gap-4 text-sm">
                          ...
                          <span className="text-gray-500 dark:text-gray-400">
                            {score.correct} correct, {score.incorrect} wrong, {score.skipped} skipped
                          </span>
                        ...
                        <div className="mt-3 space-y-2">
                          ...
                          <div className="w-full bg-amber-100 dark:bg-amber-900/30 rounded-full h-1.5">
                            <div
                              ...
                              style={{ width: `${Math.max(0, (1 - breakRemaining / BREAK_DURATION_MS)) * 100}%` }}
                            />
                          </div>
                        </div>
                      )}
                    ...
                    <div className="flex-shrink-0 mt-2 flex flex-col items-center gap-2">
                      ...
                      {status === "on-break" && (
                        <div
                          role="button"
                          tabIndex={0}
                          onClick={(e) => { e.stopPropagation(); handleSkipBreak(section); }}
                          onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.stopPropagation(); handleSkipBreak(section); } }}
