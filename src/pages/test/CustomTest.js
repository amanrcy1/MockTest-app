...
const CustomTest = () => {
  ...
  const handleNext = () => {
    ...
    if (responses[currentQuestionIndex]?.selectedAnswer && !responses[currentQuestionIndex]?.markedForReview) {
          ...
          <div className="md:hidden">
            ...
            {settings.hasTimer && (
              <div className="flex justify-center items-center bg-gray-50 dark:bg-gray-700 rounded-lg p-2">
                <p className={`text-lg font-bold ${timeRemaining < 300 ? "text-red-600 animate-pulse" : "text-blue-600"}`}>
                  {formatTime(timeRemaining)}
                </p>
              </div>
            )}
