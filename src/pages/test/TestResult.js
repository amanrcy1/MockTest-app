...
const TestResult = () => {
  ...
  const calculatePercentile = useCallback(async () => {
    ...
    try {
      ...
      const computedPercentile =
        allScores.length > 0 ? ((belowMe / allScores.length) * 100).toFixed(2) : 100;
  ...
  const totalAttempted = scoreData.correct + scoreData.incorrect + scoreData.skipped;
  ...
  const timeSummary = timeEntries.length
    ? {
        average: Math.round(
          timeEntries.reduce((sum, item) => sum + item.timeTaken, 0) / timeEntries.length
        ),
        fastest: timeEntries.reduce((min, item) => (item.timeTaken < min.timeTaken ? item : min)),
        slowest: timeEntries.reduce((max, item) => (item.timeTaken > max.timeTaken ? item : max)),
        slowestList: [...timeEntries].sort((a, b) => b.timeTaken - a.timeTaken).slice(0, 5),
      }
    : null;
  ...
  const nextAction =
    weakestTopic && weakestTopic.accuracy < 65
      ? {
          ...
          onClick: () =>
            navigate('/test-selection', {
              state: { mode: 'practice', topic: weakestTopic.topic, examType },
            }),
        }
      : {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-4">
            ...
            <p className="text-2xl font-bold text-gray-900 dark:text-white">
              {timeSummary ? formatTime(timeSummary.average) : '-'}
            </p>
          </div>
        </div>
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="text-5xl font-bold mb-2">
                ...
                <span className="text-xl opacity-70 ml-2">
                  / {responses.reduce((sum, r) => sum + (r.marksPerQuestion || 1), 0).toFixed(2)}
                </span>
              </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8">
          <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">Score Breakdown</h3>
        ...
        {/* Weak Areas */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8">
          <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">Areas to Improve</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {Object.entries(topicPerformance)
            ...
            {Object.entries(topicPerformance).filter(([_, data]) => data.correct / data.total < 0.5)
              .length === 0 && (
              ...
              <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/50 rounded-xl flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
                    Solutions are hidden for skipped questions. Use AI Explanation or retake the
                    test.
                  </p>
                </div>
                <button
                  onClick={() =>
                    navigate('/test-selection', { state: { examType, mode: testMode || 'mock' } })
                  }
                    ...
                    {isSkipped && (
                      <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/50 rounded-lg p-4 flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
                            Solution is hidden. Use AI Explanation below or retake the test.
                          </p>
                        </div>
                      </div>
                    )}
