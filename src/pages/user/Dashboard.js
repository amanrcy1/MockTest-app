...
// ── Greeting helper (IST-aware) ──
const getGreeting = () => {
  // Get current time in IST using Intl.DateTimeFormat (most robust method)
  const istHour = new Intl.DateTimeFormat('en-US', {
...
const SubjectBar = ({ subject, accuracy, count, delay = 0 }) => {
  const barColor =
    accuracy >= 70 ? 'bg-emerald-500' : accuracy >= 45 ? 'bg-amber-500' : 'bg-red-400';
  return (
    <motion.div
...
const RecentTestItem = ({ test, onClick, delay = 0 }) => {
  ...
  const timeAgo = test.endTime
    ? formatDistanceToNow(new Date(test.endTime), { addSuffix: true })
    : '';
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-shrink-0">
        ...
        />
        <span
          className={`absolute inset-0 flex items-center justify-center text-[10px] font-bold ${accColor}`}
        >
          {acc.toFixed(0)}%
        </span>
      </div>
      <div className="flex-1 text-left min-w-0">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400">
          {test.correct}/{(test.correct || 0) + (test.incorrect || 0) + (test.skipped || 0)} correct
          · {timeAgo}
        </p>
      </div>
      <svg
        className="w-4 h-4 text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300 flex-shrink-0 transition-colors"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        // Best score
        const bestScore =
          attempted > 0 ? Math.max(...userTests.map((t) => Number(t.accuracy || 0))) : 0;
  ...
  const rankDisplay = useMemo(() => {
    ...
    if (r === 1)
      return {
        ...
        badge: 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400',
      };
    if (r <= 3)
      return {
        label: `#${r} Elite`,
            ...
            >
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <p className="font-semibold text-white text-sm truncate">
                  Continue {activeSession.mode} &middot; {activeSession.examType}
                </p>
              </div>
                ...
                <button
                  onClick={() => {
                    if (activeSession.mode === 'mock')
                      navigate('/test/mock', {
                        state: { examType: activeSession.examType, resume: true },
                      });
                      ...
                      navigate('/test/custom', {
                        state: { settings: activeSession.settings, resume: true },
                      });
                  }}
          ...
          >
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                <p className="font-semibold text-white text-sm truncate">
                  CDS Mock: {cdsPaperSession.completedCount}/3 papers done
                </p>
        ...
        >
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              {getGreeting()}, {userDetails?.name?.split(' ')[0]}
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              {getMotivation(stats.attempted, stats.averageAccuracy)}
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            return (
              ...
              >
                <div
                  className={`inline-flex p-1.5 rounded-lg bg-gradient-to-br ${colors.bg} text-white mb-2`}
                >
                  {s.icon}
                </div>
        ...
        {stats.attempted === 0 && (
          ...
          >
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 max-w-xs mx-auto">
              Take your first mock test to unlock insights, rank, and performance tracking.
            </p>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] text-gray-400">
                  Last {stats.accuracyTrend.length} tests
                </span>
              </div>
              <div className="flex items-end justify-between gap-4">
                ...
                <div className="text-right">
                  <p className="text-lg font-bold text-gray-900 dark:text-white">
                    {stats.accuracyTrend[stats.accuracyTrend.length - 1]?.toFixed(0)}%
                  </p>
            ...
            >
              <div className="relative flex-shrink-0">
                ...
                <span className="absolute inset-0 flex items-center justify-center text-sm font-bold text-amber-600 dark:text-amber-400">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-gray-900 dark:text-white mt-0.5">
                  {stats.bestScore.toFixed(0)}% accuracy
                </p>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <QuickLink
              title="Custom Test"
              delay={0.55}
              onClick={() => navigate('/test/custom-setup')}
