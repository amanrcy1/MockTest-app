...
const createCroppedImage = async (imageSrc, pixelCrop, maxSize = 200, quality = 0.8) => {
  ...
  return new Promise((resolve) => {
    ...
    const tryCompress = () => {
      ...
      if ((base64.length * 0.75) / 1024 > 100 && q > 0.3) {
        q -= 0.1;
        tryCompress();
      } else resolve(base64);
    };
...
const Profile = () => {
  ...
  const handleFileSelect = async (e) => {
    ...
    if (!file.type.startsWith('image/')) {
      toast.error(messages.INVALID_IMAGE);
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      toast.error(messages.IMAGE_TOO_LARGE);
      return;
    }
  ...
  const handleCropSave = async () => {
    ...
    try {
      ...
      logger.error('Crop save error:', err);
      toast.error(messages.PHOTO_SAVE_FAILED);
    } finally {
  ...
  const handleRemovePhoto = async () => {
    ...
    try {
      ...
      setPreviewUrl(null);
      await refreshUserDetails();
      toast.success(messages.PHOTO_REMOVED);
    } catch (err) {
      logger.error('Remove photo error:', err);
      toast.error(messages.PHOTO_REMOVE_FAILED);
    } finally {
      setUploadingPhoto(false);
    }
  };
        ...
        <header className="sticky top-0 md:top-14 z-40 backdrop-blur-xl bg-white/60 dark:bg-gray-900/60 border-b border-white/20 dark:border-gray-700/30">
          <div className="px-4 py-3 max-w-2xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="p-2 -ml-2 md:hidden">
    ...
    <div className="min-h-screen relative overflow-hidden pb-20 md:pb-0">
            ...
            <div className="p-6 bg-black/80">
              <div className="flex items-center gap-4 max-w-xs mx-auto">
                ...
                <input
                  ...
                  className="flex-1 h-1 bg-white/30 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:rounded-full"
                />
                <svg
            ...
            >
              ...
              <div className="p-2">
                <button
                  ...
                  className="w-full min-h-11 flex items-center gap-4 p-4 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition-colors"
                >
                ...
                {previewUrl && (
                  <button
                    ...
                    disabled={uploadingPhoto}
                    className="w-full min-h-11 flex items-center gap-4 p-4 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors disabled:opacity-50"
                  >
              ...
              <div className="p-2 border-t border-gray-100 dark:border-gray-700">
                <button
                  onClick={() => setShowPhotoOptions(false)}
                  className="w-full min-h-11 p-4 text-gray-500 font-semibold hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl transition-colors"
                >
                  Cancel
                </button>
              </div>
        ...
        <div className="px-4 py-3 max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <motion.button
              ...
              className="p-2 min-h-11 min-w-11 -ml-2 rounded-xl hover:bg-white/40 dark:hover:bg-gray-700/40 transition-colors md:hidden"
              aria-label="Back"
            >
              <svg
      ...
      <main className="relative z-10 px-4 py-8 max-w-2xl mx-auto">
        <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
          ...
          <motion.div
            variants={fadeUp}
            className="relative bg-white/70 dark:bg-gray-900/70 backdrop-blur-2xl rounded-3xl border border-white/30 dark:border-gray-700/40 shadow-2xl shadow-purple-500/5 overflow-hidden"
          >
            ...
            <div className="px-6 pb-6 -mt-10 flex flex-col items-center">
              ...
              >
                ...
                <motion.div
                  className="absolute -bottom-1 -right-1 w-8 h-8 bg-blue-500 rounded-full border-[3px] border-white dark:border-gray-900 flex items-center justify-center shadow-lg"
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
          ...
          <motion.div
            variants={fadeUp}
            className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-2xl rounded-3xl border border-white/30 dark:border-gray-700/40 shadow-xl overflow-hidden"
          >
            <div className="p-5">
